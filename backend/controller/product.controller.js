...
import {cloudinaryupdate } from "../utils/cloudinary.js";
// create product::
//admin route:
const createproduct = asyncHandler(async (req, res, next) => {
  ...
  if (req.file) {
    try {
      ...
      if (!images)
        return res.status(501).json({ message: `error in images`, ok: false });
      const avatar = await cloudinaryupdate(images);
      console.log(avatar);
      if (!avatar.url)
        return res
          .status(501)
          .json({ message: `error in uploading iin cloudinary`, ok: false });
      const product = await Product.create({ ...req.body, images: avatar.url });
      console.log(product);
      return res
        .status(200)
        .json(
  ...
  return res
    .status(501)
    .json({ message: `file not uploaded sucessfully`, ok: false });
});
    ...
    res
      ...
      .json(
        new apiresponse(
          ...
          allproduct,
          productcount,
          "data have beeen fetched successfully"
        )
      );
  } catch (error) {
...
const updateproduct = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(new apiresponse(200, product, "data have been updated successfully"));
});
const deleteproduct = asyncHandler(async (req, res, next) => {
  let product = await Product.findById(req.params.id);
  console.log(product);
  if (!product) return next(new apierror(404, "product does not deleted"));
  await Product.deleteOne({ _id: req.params.id });
  return res
    .status(200)
    .json(new apiresponse(200, "product have been deleted"));
});
...
const productReview = asyncHandler(async (req, res, next) => {
  ...
  return res
    .status(200)
    .json(
      new apiresponse(200, product, "review have been submitted successfully")
    );
});
const findallreviews = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json(new apiresponse(200, product.reviews, "all the reviews are fetched"));
});
const deletereview = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json(
      new apiresponse(200, newproduct.reviews, " the reviews have been deleted")
    );
});

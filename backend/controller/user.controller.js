...
const forgitpassword = asyncHandler(async (req, res, next) => {
    ...
    const resetpasswordurl = `http://${req.get("host")}/api/v1/passward/reset/${resettoken}`;
    ...
    try {
        ...
        return res.status(200).json(new apiresponse(200, `email has been sent to ${user.email} sucessfully`));
    } catch (error) {
...
const resetpassword = asyncHandler(async (req, res, next) => {
    ...
    console.log(user);
    if (!user) {
        return next(new apierror(400, "reset password token is in valid or has been expired"));
    }
...
const updatepassword = asyncHandler(async (req, res, next) => {
    ...
    if (!check)
        return next(new apierror(400, "old password is not corrrect"));
    if (req.body.newpassward !== req.body.confirmpassward)
        next(new apierror(401, "new passward anf=d confirm apssward sholud be same"));
...
const updateprofile=asyncHandler(async(req,res,next)=>{
    ...
    res.status(200).json(new apiresponse(200, user,"your profile have beeen updated successfully"))
})
//admin routes to get of all details::
const getallusers=asyncHandler(async(req,res,next)=>{
    const users=await User.find();
    res.status(200).json(new apiresponse(200,users,"all the users are fetched succesfully bawa"));
})
...
const getdetailsofoneuser=asyncHandler(async(req,res,next)=>{
    ...
    if(!user)
    {
        next(new apierror(401,`user does not exixts with the spefic id ${req.params._id}`))
    }
    console.log(user);
    res.status(200).json(new apiresponse(200,user,"specfic users is fetched succesfully bawa"));
})
//total no of user::
const updaterole=asyncHandler(async(req,res,next)=>{
    ...
    const user=await User.findByIdAndUpdate(req.params.id, {$set: {role:req.body.role}},{
        new:true
    })
    res.status(200).json(new apiresponse(200, user,"your profile have beeen updated successfully"))
})
const deleteuser=asyncHandler(async(req,res,next)=>{
    ...
    res.status(200).json(new apiresponse(200, user,"user have been deleted  successfully"))

})

export { Register, Loginuser, logout, forgitpassword,
     resetpassword, getuserprofile,updatepassword,updateprofile,
    getallusers,getdetailsofoneuser,updaterole,deleteuser
    }